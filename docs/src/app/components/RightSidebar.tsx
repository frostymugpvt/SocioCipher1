...
export default function RightSidebar({ user, selectedUserBadge, onTrust }: RightSidebarProps) {
  ...
  return (
    <aside className="right-sidebar">
      {/* TRUST INDEX */}
      <section style={{ padding: '1.25rem', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div className="mono" style={{ fontSize: '0.6rem', color: '#555', letterSpacing: '2px', marginBottom: '0.75rem' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
          ...
          <div className="mono" style={{ fontSize: '0.55rem', color: '#444', marginTop: '0.25rem' }}>REPUTATION_WEIGHT</div>
        </div>
        ...
        {selectedUserBadge && selectedUserBadge !== user?.badgeNumber ? (
          ...
          <div className="mono" style={{ fontSize: '0.6rem', color: '#333', textAlign: 'center' }}>
            CLICK_USER_IN_FEED_TO_VOUCH
          </div>
        )}
        <div className="mono" style={{ fontSize: '0.55rem', color: '#333', textAlign: 'center', marginTop: '0.5rem' }}>
      ...
      <section style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
          <div className="mono" style={{ fontSize: '0.6rem', color: '#555', letterSpacing: '2px' }}>
        ...
        {activeThread ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div
              ...
              style={{ padding: '0.5rem 1.25rem', background: '#0a0a0a', cursor: 'pointer', fontSize: '0.7rem', color: '#555', borderBottom: '1px solid var(--color-border)' }}
            >
              ← {activeThread}
            </div>
                ...
                threadMessages.map(msg => (
                  <div
                    ...
                    style={{
                      ...
                      maxWidth: '85%',
                      background: msg.direction === 'sent' ? 'rgba(139,0,0,0.15)' : '#111',
                      border: `1px solid ${msg.direction === 'sent' ? 'rgba(139,0,0,0.3)' : '#1a1a1a'}`,
                  ...
                  >
                    <div>{msg.content}</div>
                    <div className="mono" style={{ fontSize: '0.55rem', color: '#444', marginTop: '0.25rem', textAlign: 'right' }}>
          ...
          <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto' }}>
            {conversations.length === 0 ? (
              <div className="mono" style={{ padding: '2rem', textAlign: 'center', fontSize: '0.6rem', color: '#333' }}>
              ...
              conversations.map(conv => (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#555', marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '160px' }}>
                      {conv.last_message}
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.25rem' }}>
                    <span className="mono" style={{ fontSize: '0.55rem', color: '#444' }}>{getRelTime(conv.last_at)}</span>
                    {conv.unread_count > 0 && (
        ...
        {/* Compose */}
        <div style={{ padding: '0.75rem', borderTop: '1px solid rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          {!activeThread && (
            <input
              type="text"
              ...
              style={{ background: '#000', border: '1px solid #1a1a1a', color: 'var(--color-accent-primary)', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', padding: '0.4rem 0.6rem', width: '100%', outline: 'none' }}
            />
          )}
          <textarea
            ...
            rows={2}
            style={{ background: '#000', border: '1px solid #1a1a1a', color: '#ccc', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', padding: '0.4rem 0.6rem', width: '100%', resize: 'none', outline: 'none' }}
          />
          <button
            onClick={handleSend}
